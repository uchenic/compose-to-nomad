...
class ComposeProcessor(object):
    ...
    def gen_service_task(self, task_name, service,group_name=None):
        ...
        task['config']={
            'image': service.get('image',self.docker_registry+task_name) ,
            'ports': ['{}'.format(x['published']) for x in service['ports']] if 'ports' in service.keys() else [],
            'network_mode': list(service['networks'].keys())[0],
            'network_aliases': [task_name]
        }
        for x in (service['ports'] if 'ports' in service.keys() else []):
            self.ports_for_groups[group_name]['port'][x['published']]={'to': x['target'], 'static': x['published']}
        limits = service['deploy']['resources']['limits']
        task['resources'] = {
            'cpu': int(float(limits['cpus'])* 1000) ,
            'memory': int(limits['memory'][:-1])
        }
        #TODO: process "depends on" https://www.nomadproject.io/docs/job-specification/lifecycle#init-task-pattern
    ...
    def gen_network_task(self, network_name):
        ...
        task['config']={
        'command':'/bin/sh',
        'args':['-c', 'docker network create {} || exit 0'.format(network_name)]
        }
        return '{}_net_init'.format(network_name), task

    def gen_service_tasks(self, services, group_name=None):
        tasks = dict([  self.gen_service_task(task_name,task,group_name)   for (task_name,task) in services.items()])
        return tasks

    def gen_group(self,group_name):
        group = {}
        #TODO: add volume creation tasks
        group['task']= self.gen_service_tasks(self.compose_obj['services'],group_name)
